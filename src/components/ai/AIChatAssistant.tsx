...
import { buildTransactionContext, streamAIResponse } from '@/services/aiService';
...
const AIChatAssistant: React.FC<Props> = ({ transactions }) => {
    ...
    <Card className="border-border/60 flex flex-col h-[600px]">
      <div className="p-4 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">AI Financial Analyst</h3>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-xs mt-1">e.g. "What are my top expenses this month?" or "Compare revenue by account"</p>
          </div>
        )}
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
            ...
            <div className={`max-w-[80%] ${msg.role === 'user' ? 'bg-primary text-primary-foreground rounded-2xl rounded-br-md px-4 py-2.5' : ''}`}>
              {msg.role === 'user' ? (
      ...
      <div className="p-3 border-t border-border">
        <div className="flex gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 resize-none px-3 py-2 text-sm border border-input rounded-lg bg-background focus:outline-none focus:ring-1 focus:ring-ring"
          />
          <button
            onClick={send}
