...
AS $$
  SELECT
    ...
    ROUND(COALESCE(SUM(CASE
      WHEN type = 'Outflow' AND account IN ('Wise PORTEPARIS LTD','Wise ME24','Wise TalenHaten','Wise YOURANWEI LTD','Airwallex ME24','Airwallex TalenHaten','CIH Ilias','CIH Kaoutar','Alison','Ki2powers')
      THEN amount * CASE currency WHEN 'EUR' THEN 1.0 WHEN 'USD' THEN 0.92 WHEN 'HKD' THEN 0.118 WHEN 'GBP' THEN 1.17 ELSE 1.0 END
    END), 0), 2) AS opex_eur,
    ROUND(
      COALESCE(SUM(CASE
