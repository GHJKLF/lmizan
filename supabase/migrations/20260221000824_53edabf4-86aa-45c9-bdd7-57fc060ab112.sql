...
AS $function$
  ...
  -- Airwallex balances from the API (source of truth)
  airwallex_api_balances AS (
    SELECT
      ac.account_name AS account,
      upper(ab.currency) AS currency,
      ab.total_amount AS running_balance,
      ab.available_amount AS balance_available,
      (ab.total_amount - ab.available_amount) AS balance_reserved,
      (ab.synced_at::date) AS date
  ...
  latest_with_rb AS (
    ...
    WHERE t.user_id = auth.uid()
      ...
      -- Exclude Airwallex accounts handled by API balances
      AND NOT EXISTS (
        SELECT 1 FROM airwallex_connections axc
        WHERE axc.user_id = auth.uid() AND axc.account_name = t.account
      )
    ORDER BY t.account, upper(t.currency), t.date DESC, t.created_at DESC
  ),
  stripe_api_balances AS (
    SELECT
      sc.account_name AS account,
      upper(sc.currency) AS currency,
      (sc.balance_available + sc.balance_pending) AS running_balance,
      sc.balance_available AS balance_available,
      sc.balance_pending AS balance_reserved,
      (sc.balance_fetched_at::date) AS date
  ...
  computed_balances AS (
    ...
    WHERE t.user_id = auth.uid()
      ...
      AND NOT EXISTS (
        SELECT 1 FROM stripe_api_balances sab
        WHERE sab.account = t.account
      )
      -- Exclude Airwallex accounts handled by API balances
      AND NOT EXISTS (
        SELECT 1 FROM airwallex_connections axc
  ...
  latest AS (
    SELECT * FROM airwallex_api_balances
    UNION ALL
    SELECT * FROM latest_with_rb
    UNION ALL
    SELECT * FROM stripe_api_balances
    UNION ALL
