...
AS $function$
  ...
  -- For accounts WITH running_balance, use latest row
  latest_with_rb AS (
  ...
  -- For accounts WITHOUT running_balance, compute net from sum of flows
  computed_balances AS (
    SELECT
      t.account,
      upper(t.currency) AS currency,
      SUM(CASE WHEN t.type = 'Inflow' THEN t.amount ELSE -t.amount END) AS running_balance,
