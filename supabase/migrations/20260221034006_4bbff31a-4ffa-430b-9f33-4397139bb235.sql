...
AS $function$
  ...
  -- Stripe accounts covered by API balance snapshot
  stripe_accounts AS (
    SELECT account_name FROM stripe_connections
    WHERE user_id = auth.uid() AND balance_fetched_at IS NOT NULL
  ),

  -- Airwallex accounts covered by balances table
  airwallex_accounts AS (
    SELECT account_name FROM airwallex_connections WHERE user_id = auth.uid()
  ),

  -- Accounts that have running_balance data (exclude Stripe API and Airwallex API accounts)
  rb_accounts AS (
    SELECT DISTINCT account
    FROM transactions
    WHERE user_id = auth.uid()
      ...
      AND account NOT IN (SELECT account_name FROM stripe_accounts)
      AND account NOT IN (SELECT account_name FROM airwallex_accounts)
  ),

  -- Computed accounts: no running_balance, not Stripe API, not Airwallex API
  ...
  -- Strategy 1: running_balance accounts (Wise, WorldFirst, CIH, CFG, etc.)
  rb_daily AS (
    SELECT
      ds.date,
      sub.account,
  ...
  -- Strategy 2: computed accounts (PayPal — cumulative Inflow - Outflow, excluding Transfer)
  computed_daily AS (
    SELECT
      ds.date,
      t.account,
  ...
  -- Strategy 3: Stripe from API balance (current snapshot, flat for recent dates)
  stripe_bal AS (
    SELECT
      ds.date,
      sc.account_name AS account,
      upper(sc.currency) AS currency,
      (sc.balance_available + COALESCE(sc.balance_pending, 0)) AS balance
    FROM stripe_connections sc
    CROSS JOIN date_spine ds
    WHERE sc.user_id = auth.uid()
      AND sc.balance_fetched_at IS NOT NULL
      -- Only show from when they have data
      AND ds.date >= (sc.balance_fetched_at::date - 30)
  ),

  -- Strategy 4: Airwallex from balances table (current snapshot)
  airwallex_bal AS (
    SELECT
      ds.date,
  ...
  combined AS (
    ...
    UNION ALL
    SELECT date, account, currency, balance FROM stripe_bal
    UNION ALL
